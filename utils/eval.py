...
import sys
sys.path.append('.')
from ChatQformer.utils.box_utils import box3d_iou, construct_bbox_corners
import re
...
from ChatQformer.utils.helper import clean_answer, answer_match, scanrefer_get_unique_multiple_lookup
...
def calc_scanrefer_score(preds, config=None):
    # 加载实例属性和ScanNet属性文件, 包含场景中物体的位置信息
    ...
    for i, output in enumerate(preds):
        ...
        obj_id = output["gt_id"]
        instance_locs = instance_attrs[scene_id]["locs"]  # 实例位置
        scannet_locs = scannet_attrs[scene_id]["locs"]  # ScanNet位置
        unique_multiple = unique_multiple_lookup[scene_id][str(obj_id)]
        spious.append(output["spiou"])
        if unique_multiple == 0:
            unique_all += 1  # 单实例场景
        else:
            multiple_all += 1  # 多实例场景
        # 预测ID提取
        pred = output["pred"]
    ...
    # 新增：计算分割相关的指标
    spious = np.array(spious)
    precision_quarter = (spious > 0.25).sum().astype(float) / spious.size
    precision_half = (spious > 0.5).sum().astype(float) / spious.size
    miou = spious.mean()
    val_scores = {
        '[scanrefer] Acc@0.25': float(iou25_acc) / len(preds),
        '[scanrefer] Acc@0.50': float(iou50_acc) / len(preds),
        '[scanrefer] Unique Acc@0.25': float(unique_iou25_acc) / unique_all if unique_all > 0 else 0.0,
        '[scanrefer] Unique Acc@0.50': float(unique_iou50_acc) / unique_all if unique_all > 0 else 0.0,
        '[scanrefer] Multiple Acc@0.25': float(multiple_iou25_acc) / multiple_all if multiple_all > 0 else 0.0,
        '[scanrefer] Multiple Acc@0.50': float(multiple_iou50_acc) / multiple_all if multiple_all > 0 else 0.0,
        '[scanrefer] Seg Precision@0.25': precision_quarter,
        '[scanrefer] Seg Precision@0.5': precision_half,
        '[scanrefer] Seg mIoU': miou
    }
...
def calc_referit3d_score(preds, eval_name, config=None):
    ...
    val_scores = {
        ...
        f'[{eval_name}] Seg Precision@0.25': precision_quarter,
        f'[{eval_name}] Seg Precision@0.5': precision_half,
        f'[{eval_name}] Seg mIoU': miou
    }
...
def calc_scan2cap_score(preds, tokenizer, scorers, config=None):
    ...
    precision_half = (spious > 0.5).sum().astype(float) / spious.size
    miou = spious.mean()
    val_scores["[scan2cap] Seg Precision@0.25"] = precision_quarter
    val_scores["[scan2cap] Seg Precision@0.5"] = precision_half
    val_scores["[scan2cap] Seg mIoU"] = miou
    return val_scores


def calc_scanqa_score(preds, tokenizer, scorers, config=None):
    val_scores = {} # 创建空字典存储各种评分
    tmp_preds = {}  # 临时存储预测结果
    tmp_targets = {} # 临时存储目标答案
    acc, refined_acc = 0, 0 # 精确匹配和精细匹配
    print("Total samples:", len(preds))
    # 遍历所有预测结果
    spious = []
    for i, output in enumerate(preds):
        # 创建唯一的项目ID，由场景ID、真实ID、问题ID和索引组成
        item_id = f"{output['scene_id']}_{output['gt_id']}_{output['qid']}_{i}"
        pred = output["pred"] # 获取预测结果
        # 处理预测文本：如果长度大于1
        if len(pred) > 1:
            if pred[-1] == '.':
                pred = pred[:-1]
    ...
    precision_half = (spious > 0.5).sum().astype(float) / spious.size
    miou = spious.mean()
    val_scores[f"[scanqa] Seg Precision@0.25"] = precision_quarter
    val_scores[f"[scanqa] Seg Precision@0.5"] = precision_half
    val_scores[f"[scanqa] Seg mIoU"] = miou
    return val_scores
...
def calc_sqa3d_score(preds, tokenizer, scorers, config=None):
    ...
    precision_half = (spious > 0.5).sum().astype(float) / spious.size
    miou = spious.mean()
    val_scores[f"[sqa3d] Seg Precision@0.25"] = precision_quarter
    val_scores[f"[sqa3d] Seg Precision@0.5"] = precision_half
    val_scores[f"[sqa3d] Seg mIoU"] = miou
    return val_scores
