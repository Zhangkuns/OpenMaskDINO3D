...
from ChatQformer.dataset import MetaLoader, create_dataset, create_loader, create_sampler
from ChatQformer.dataset.dataset_train import train_collate_fn
from ChatQformer.dataset.dataset_val import val_collate_fn
from ChatQformer.models.chat3d import Chat3D
from ChatQformer.tasks.shared_utils import get_media_types, setup_model
from ChatQformer.utils.basic_utils import (MetricLogger, SmoothedValue, setup_seed)
from ChatQformer.utils.config_utils import setup_main
from ChatQformer.utils.distributed import get_rank, get_world_size, is_main_process
from ChatQformer.utils.logger import log_dict_to_wandb, setup_wandb
from ChatQformer.utils.eval import calc_scanrefer_score, clean_answer, calc_scan2cap_score, calc_scanqa_score, calc_sqa3d_score, calc_multi3dref_score, calc_referit3d_score, calc_scanrefer_location_score, calc_multi3dref_location_score
from ChatQformer.models.seg_loss import get_iou
...
def train(
    ...
    # 定义需要监控的损失类型
    loss_names = ["loss", "obj_norm", "obj_img_norm", "objid_norm", "scene_norm", "text_feat_norm", "seg_loss", "llama_loss"]
    # 获取训练数据加载器中的媒体类型
    ...
    for i, (media_type, batch) in enumerate(iterator):
        ...
        # 通过 GradScaler 缩放损失值后进行反向传播，以便在混合精度训练中保持数值稳定性
        scaler.scale(loss).backward()

        if ((i + 1) % accum_iter == 0) or (i + 1 == len(train_loader)):
            # 检查是否需要梯度裁剪（Gradient Clipping）。
        ...
        # 将当前优化器的学习率更新到 metric_logger 中。
        metric_logger.update(lr=optimizer.param_groups[-1]["lr"])

        # 在主进程中、启用 wandb（Weights & Biases）并到达日志记录频率时，执行日志上传操作。
        ...
        if do_eval and ((i+1) % eval_freq == 0 and (len(train_loader) - i >= eval_freq) or i == len(train_loader) - 1):
            if is_main_process():
                # 创建一个字典 param_grad_dic，记录模型中所有参数是否需要梯度。
                param_grad_dic = {
                ...
                print(f"Save model at epoch {epoch} step {global_step}")
            val_metrics = evaluate_all(model, model_without_ddp, val_loaders, epoch, global_step, device, config)
            # is_main_process() 用于判断当前进程是否为主进程。
            if is_main_process():
                for k, v in val_metrics.items():
                    # eval_metric_logger 是一个日志工具，包含所有已注册的指标。
                    # 如果当前k未注册，则注册新指标
...
def evaluate_all(
    ...
    model.eval()
    # 配置模型使用缓存（通常是注意力计算的中间结果）来加速推理阶段。
...
def evaluate(
    ...
    if config.distributed:
        # 如果是分布式训练，使用 set_epoch 确保验证数据加载的随机性和分布一致性。
    ...
    for i, batch in tqdm(enumerate(val_loader)):
        ...
        if "custom_prompt" in batch:
            ...
            pred_output_texts = pred[0]
            pred_output_masks = pred[1]
            pred_output_scores = pred[2]
            for bi in range(batch_size):
                ...
                object_sp_mask = batch['object_sp_masks'][bi]
                tmp_pred = pred_output_texts[bi]
                ref_caption = batch["ref_captions"][bi]
                pred_mask = pred_output_masks[bi].to(object_sp_mask.device)
                pred_score = pred_output_scores[bi]
                spiou = get_iou(pred_mask, object_sp_mask)
                spiou = float(spiou.item())
                save_preds.append({
                    "scene_id": scene_id,
                    "gt_id": obj_id,
                    'pred_id': pred_id,
    ...
    # if is_main_process():
    #     save_preds = []
    #     # 主进程收集所有 rank 的结果，并合并到一个 JSON 文件中。
    ...
    # 根据不同数据集名称（eval_name），调用对应的评估函数计算指标。
    val_scores = {}
...
def setup_dataloaders(config):
    ...
    train_loaders = create_loader(
        ...
        # 指定如何将数据样本合并成批次（train_collate_fn 是训练用的 collate 函数）
