...
from ChatQformer.prompts.prompts import scan2cap_location_prompt
import nltk
...
for split in ["train", "val"]:
    ...
    for scene_id in tqdm(scene_ids):
        ...
        for gt_id, pred_id in enumerate(gt_match_id):
            ...
            count[pred_id] += 1
            pred_locs = instance_locs[pred_id].tolist()
            loc_tokens = [num_to_location_token(x) for x in pred_locs]
            loc_caption = "<LOCATION> " + " ".join(loc_tokens) + " </LOCATION>"
            if split == 'train':
                for caption in corpus[f"{scene_id}|{gt_id}"]:
                    new_annos.append({
                        ...
                        'obj_id': gt_id,
                        'pred_id': pred_id,
                        'prompt': random.choice(scan2cap_location_prompt).replace(f"<loc>", loc_caption),
                        "caption": caption,
    ...
    with open(f"annotations/scan2cap_{segmentor}_{split}_location{version}.json", "w") as f:
