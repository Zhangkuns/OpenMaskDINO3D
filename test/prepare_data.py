...
def process_per_scan(scan_id, scan_dir, out_dir, tmp_dir, apply_global_alignment=True, is_test=False):
    ...
    superpoint_semantic_labels = torch.full((len(unique_sp_ids),), -100, dtype=torch.int64)  # 默认为-100（忽略）
    ...
    for i, sp_id in enumerate(unique_sp_ids):
        ...
        if len(valid_labels) > 0:
            ...
            superpoint_semantic_labels[i] = most_common_label
    ...
    torch.save(
        (coords, coords_align, colors, sem_labels, instance_labels, superpoint,
         ...
         gt_instance_labels, gt_segids, gt_instance_sp_masks, gt_instance_features
         ),
