# 设置工作目录
cd /workspace/Mask3Dformer/ChatQformer
# 添加项目根目录到 PYTHONPATH
export PYTHONPATH=/workspace/Mask3Dformer:$PYTHONPATH
...
pretrained_path="/workspace/data/checkpoints/ckpt_05_40466_img_epoch2.pth"
OUTPUT_DIR=outputs/"$(date +"%Y%m%d_%H%M%S")"_lr"$lr"_ep"$epoch"_"$tag"
mkdir -p ${OUTPUT_DIR}

python tasks/train.py \
    "./scripts/config_maskformer_chaos.py" \
